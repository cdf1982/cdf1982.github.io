...
To connect to IP cameras on your local network, GlanceCam requests the macOS Local Network permission. This permission allows the app to discover and communicate with devices on your network. No data about your local network or the devices on it is collected or transmitted by the app.

GlanceCam includes an optional feature that uses Touch ID or your macOS login password to restrict access to the app's Settings. Biometric authentication is handled entirely by Apple's LocalAuthentication framework; GlanceCam never has access to your biometric data and does not transmit it anywhere.
...
*Last revision: April 19, 2026 (added paragraphs about Local Network permission and Touch ID / password lock for Settings)*